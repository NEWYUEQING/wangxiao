    ...
    def update_reconstruction(self, event_tensor, event_tensor_id=None, stamp=None):
        with torch.no_grad():
            ...
            with CudaTimer('Reconstruction'):
                ...
                output_events = self.crop.pad(events)
                # if np.isnan(output_events.cpu()).any():
                #     pdb.set_trace()
                #     print("=====")
                   #continue
                ########################################
                ...
                for channel in events_for_each_channel.keys():
                    with CudaTimer('Inference'):
                        new_predicted_frame, states, output_feats = self.model(events_for_each_channel[channel],
